...
class RestClient {
  ...
  get = (successCB, errorCB) => {
    this.handleRequest(false, successCB, errorCB, null);
  };

  /**
   * This method is used to make post call to the services. It takes 2 parameters, successCB,
   * errorCB and based on the response it will call success or error callbacks.
   * @param successCB
   * @param errorCB
   * @param requestBody
   * @returns json response for the desired request
  ...
  post = (successCB, errorCB, requestBody) => {
    this.handleRequest(true, successCB, errorCB, requestBody);
  };

  handleRequest = (isPost, successCB, errorCB, requestBody) => {
    let requestHeaders = {
